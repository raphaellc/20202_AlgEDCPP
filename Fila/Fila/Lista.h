...
template<typename tipoElemento>
inline void Lista<tipoElemento>::removerPrimeiroElemento()
{
	//se lista vazia nao faz nada

	if (inicio_lista != nullptr) {
		No<tipoElemento> * aux;
		aux = inicio_lista;
		inicio_lista = inicio_lista->getProximo();
		inicio_lista->setAnterior(final_lista);
		delete aux;
	}
	qtd_elementos--;
...
tipoElemento * Lista<tipoElemento>::buscarElementoPos(int pos)
{
	...
	return nullptr;//ou lista � vazia, ou posi��o � maior que quantidade de elementos
